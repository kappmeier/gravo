...
Public Class xlsTestGroup
  ...
  Overrides Sub Start(ByVal Group As String)
    ...
    If TestSetPhrases And TestMarked Then ' nur markierte und Phrasen einschließen
      'command = "SELECT W.Index FROM DictionaryWords AS W, " & AddHighColons(Group) & " AS G WHERE W.Index = G.WordIndex AND G.Marked = " & GetDBEntry(True) & ";"
      'command = "SELECT W.Index FROM DictionaryWords AS W, GroupEspresso101 AS G WHERE(((W.Index) = [G].[WordIndex]) And ((G.Marked) = " & GetDBEntry(True) & "))"
      ' TODO irgendwie die anführungszeichen reinbekommen??? evtl. aufräumen
    ...
    Else  ' alle auswählen und Phrasen ausschließen
      command = "SELECT W.Index FROM DictionaryWords AS W, " & AddHighColons(Group) & " AS G WHERE W.Index = G.WordIndex AND (NOT W.WordType=5);"
    End If
