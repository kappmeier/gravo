...
Public Class WordInput
    Dim db As New SQLiteDataBaseOperation                 ' Datenbankoperationen f�r Microsoft Access Datenbanken
    ...
    Private Sub WordInput_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ' Sprachen in die Listen einf�gen
    ...
    Private Sub AddSubEntry(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAddSubEntry.Click
        If chkDirectAdd.Checked And GroupEntry Is Nothing Then
            MsgBox("Bitte w�hlen sie eine existierende Gruppe aus. Eintrag wird nicht erstellt!", MsgBoxStyle.Information, "Warnung")
            Exit Sub
        End If
        ...
        ' Falls keine Sprache in der Gruppe vorhanden ist (0 Eintr�gee bisher), nachfragen ob wirklich erstellt werden soll
        If languageCount = 0 Or mainLanguageCount = 0 Then
            If MsgBox("Es ist bisher noch kein Eintrag in der gew�hlten Gruppe vorhanden. Soll ein neuer Eintrag mit den Sprachen '" & language & "' und '" & mainLanguage & "' erstellt werden?", MsgBoxStyle.YesNo, "Neue Sprache") = MsgBoxResult.No Then Exit Sub
        End If
        ...
        Catch ex As EntryNotFoundException
            ' Da der Haupteintrag nicht vorhanden ist, mu� hier auch nicht auf die xlsExists-Exception geachtet werden.
            Dim res As MsgBoxResult = MsgBox("Der Haupteintrag " & txtMainEntry.Text & " ist f�r die gew�hlten Sprachen nicht vorhanden. Soll er erstellt werden?", MsgBoxStyle.YesNo, "Haupteintrag nicht vorhanden")
            If res = MsgBoxResult.Yes Then
                ' Hinzuf�gen. Da die nicht-existiert-exception auftrat, kann nicht mehr die existiert-schon-exception auftreten
                Try
                ...
                End Try
                ' Erneut den subentry hinzuf�gen
                Try
                    DictionaryDao.AddSubEntry(deWord, txtMainEntry.Text, language, mainLanguage)
                    ' hinzuf�gen in die gruppe
                    If chkDirectAdd.Checked Then AddToGroup()
                Catch sex As EntryExistsException
                    If chkDirectAdd.Checked Then AddToGroup() ' da es schon vorhanden ist, kann es in die aktuelle Gruppe hinzugef�gt werden
                Catch sex As Exception
                    MsgBox("Eintrag nicht m�glich, konflikt mit Index wahrscheinlich. �berpr�fen Sie Ihre Datenbankversion." & vbCrLf & "Fehler: " & ex.Message, MsgBoxStyle.Critical, "Fehler")
                End Try
            Else
    ...
    Private Sub AddToGroup()
        ' Davon ausgehen, da� das Einf�gen in die Wortliste korrekt erfolgt ist
    ...
    Private Sub cmbLanguages_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbLanguages.SelectedIndexChanged
        ' Liste der Wortarten f�llen (immer alle unterst�tzen zur zeit)
